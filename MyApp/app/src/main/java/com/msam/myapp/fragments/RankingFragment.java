...
import com.msam.myapp.adapters.AdaptadorResultados;
...
public class RankingFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        consultarResultados("select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and j.id="+PreferenciasJuego.jugadorId);
            ...
            public void onClick(View v) {
                ...
                if(!tipoConsulta.equals("Individual")){
                    if(nivelJuego.equalsIgnoreCase("Todos") && modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id";
                    }else if(!nivelJuego.equalsIgnoreCase("Todos") && modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and p.nivel='"+nivelJuego+"'";
                    }else if(nivelJuego.equalsIgnoreCase("Todos") && !modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and p.modo='"+modo.toUpperCase().trim()+"'";
                    }else if(!nivelJuego.equalsIgnoreCase("Todos") && !modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and p.nivel='"+nivelJuego+"' and p.modo='"+modo.toUpperCase().trim()+"'";
                    }
                }else{
                    if(nivelJuego.equalsIgnoreCase("Todos") && modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and j.id="+jugadorId;
                    }else if(!nivelJuego.equalsIgnoreCase("Todos") && modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and j.id="+jugadorId+" and p.nivel='"+nivelJuego+"'";
                    }else if(nivelJuego.equalsIgnoreCase("Todos") && !modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and j.id="+jugadorId+" and p.modo='"+modo.toUpperCase().trim()+"'";
                    }else if(!nivelJuego.equalsIgnoreCase("Todos") && !modo.equalsIgnoreCase("Todos")){
                        consulta="select j.id,j.nombre,j.edad,j.avatar,p.puntos,p.nivel,p.modo from "+Utilidades.TABLA_JUGADOR+" j,"+Utilidades.TABLA_PUNTAJE+" p where j.id=p.id and j.id="+jugadorId+" and p.nivel='"+nivelJuego+"' and p.modo='"+modo.toUpperCase().trim()+"'";
                    }
                }
    ...
    private void consultarResultados(String consulta) {
        ...
        while (cursor.moveToNext()){
            ...
            resultados.setEdad(cursor.getString(2));
